...
- **不用 Electron**: pywebview 轻量替代，用系统原生 WKWebView
...
├── main.py                          # CLI 入口（仅 Engine，无 Dashboard）
├── openclaw_bridge.py               # 门铃接收器（FastAPI，已集成到 dashboard_api）
...
- **门铃**：TRIGGER 时 POST 到 `OPENCLAW_BRIDGE_URL`（已集成到 dashboard_api.py 的 `/emergency` 端点，端口 8089）
