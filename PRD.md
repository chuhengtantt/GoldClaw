...
> 版本: v0.1.0 | 日期: 2026-04-09 | 状态: Draft
...
- 单进程 Python 程序，无服务器、无外部数据库服务、无 GUI
- 通过 `python main.py` 命令行启动
- 所有数据存储在本地 SQLite 单文件（`data/goldclaw.db`）中
- 与外部 LLM 系统通过 Webhook HTTP 通信，不共享文件
...
| # | 功能 | 说明 |
|---|------|------|
| F19 | 前端可视化 Dashboard | 风格化 Web 前端展示投资者状态、盈亏曲线、持仓变化 |
| F20 | 前后端接口规范 | Python 引擎暴露 REST API（FastAPI），前端通过标准 HTTP 接口获取数据 |
| F21 | 自动化社媒运营 | 自动生成交易摘要、盈亏截图、投资故事，发布到 Twitter/X、Telegram 等社交平台 |
| F22 | 社媒内容模板 | 可编辑的社媒发布模板（包含投资者表现、金评、每日/周报），支持定时发布 |
| F23 | 历史回测 | 导入历史金价数据进行策略回测 |
| F24 | Telegram/Discord 通知 | 除 Webhook 外的即时通讯通知 |
| F25 | 多币种支持 | 除 XAU 外支持白银、原油等 |
| F26 | 投资者画像热更新 | 运行中修改投资者的费率/杠杆参数 |
| F27 | 交易报告生成 | 定期生成 PDF/HTML 交易报告 |

> **前后端分离预留**: v1.0 采用 SQLite + Webhook，但 JSON Schema 设计时需考虑未来 REST API 的兼容性。Python 引擎的核心逻辑（盈亏计算、状态机）应保持与数据传输层解耦，便于将来将 `webhook_client.py` 替换为 FastAPI 端点而不改动业务逻辑。
...
2. **不做 GUI / Web 界面**: 无 Dashboard，所有交互通过 Webhook 和终端日志
3. **不做用户认证**: 本地单用户运行，无登录系统
5. **不做实时推送**: 没有 WebSocket/SSE，OpenClaw 只能通过读 JSON 文件获取状态
...
| v0.2 | 集成测试：完整交易生命周期模拟 | 待开发 |
| v1.0 | 完整系统 + OpenClaw 联调 | 待开发 |
